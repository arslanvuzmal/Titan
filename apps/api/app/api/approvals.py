...
class ApprovalDecision(BaseModel):
    ...
    workflow_id: str = Field(..., description="The Temporal Workflow ID waiting for the signal")
    edited_parameters: Optional[Dict[str, Any]] = Field(None, description="New params if EDITED")
...
async def decide_approval(action_id: str, payload: ApprovalDecision, user: dict = Depends(get_current_user)):
    """
    ...
    if payload.decision not in ["APPROVED", "REJECTED", "EDITED"]:
        raise HTTPException(status_code=400, detail="Invalid decision. Must be APPROVED, REJECTED, or EDITED.")
    ...
    try:
        ...
        return {"status": "success", "message": f"Signal sent to workflow {payload.workflow_id}"}
        
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to communicate with Temporal: {str(e)}")

...
class AgentExecutionWorkflow:
    ...
    async def run(self, event_data: Dict[str, Any]) -> Dict[str, Any]:
        workflow.logger.info(f"Starting LangGraph agent execution for event: {event_data.get('event_id')}")
        ...
        if final_state.get("current_step") == "PENDING_APPROVAL":
            workflow.logger.info("Graph yielded PENDING_APPROVAL. Pausing workflow for HITL signal.")

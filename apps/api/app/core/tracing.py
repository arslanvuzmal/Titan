...
class TitanLangGraphCallbackHandler(BaseCallbackHandler):
    ...
    def on_llm_start(
        ...
        logger.info(
            f"[TRACING] LLM Start | Task: {self.task_id} | Org: {self.organization_id}"
        )
    ...
    def on_llm_error(self, error: Exception, **kwargs: Any) -> Any:
        logger.error(
            f"[TRACING] LLM Error | Task: {self.task_id} | Error: {str(error)}"
        )
    ...
    def on_chain_error(self, error: Exception, **kwargs: Any) -> Any:
        ...
        logger.error(
            f"[TRACING] Node Error | Task: {self.task_id} | Error: {str(error)}"
        )

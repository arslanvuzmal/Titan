...
class WebSearchTool(BaseTool):
    ...
    def description(self) -> str:
        return (
            "Performs a web search to gather recent information on a topic or company."
        )
    ...
    def _validate_url_for_ssrf(self, target_url: str) -> bool:
        ...
        try:
            ...
            if (
                ...
                logger.error(
                    f"SSRF Attempt blocked: {hostname} resolved to internal IP {ip_address}"
                )
                return False
    ...
    async def execute(
        ...
        if not api_key:
            return ToolResult(
                status="FAILED", message="Missing SERPER_API_KEY secret in context."
            )
        ...
        if not self._validate_url_for_ssrf(target_api_url):
            return ToolResult(
                status="FAILED",
                message="Security Exception: Invalid target URL for search API.",
            )
        ...
        async with httpx.AsyncClient() as client:
            try:
                response = await client.post(
                    target_api_url, json=payload, headers=headers, timeout=10.0
                )
                ...
                if response.status_code == 200:
                    ...
                    return ToolResult(
                        status="FAILED", message=f"Serper API Error: {response.text}"
                    )

            except Exception as e:
                return ToolResult(
                    status="FAILED", message=f"Network error executing search: {str(e)}"
                )

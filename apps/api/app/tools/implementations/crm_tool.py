...
class CRMParams(BaseModel):
    ...
    properties: Dict[str, Any] = Field(
        default_factory=dict, description="Properties to update (e.g., lead_score)."
    )
...
class CRMTool(BaseTool):
    ...
    async def execute(self, params: CRMParams, context: ToolContext) -> ToolResult:
        ...
        if not api_key:
            return ToolResult(
                status="FAILED", message="Missing HUBSPOT_API_KEY secret in context."
            )

        logger.info(
            f"[{context.task_id}] Executing CRM action '{params.action_type}' for {params.contact_email}"
        )
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                if search_res.status_code != 200 or not search_data.get("results"):
                    return ToolResult(
                        status="FAILED",
                        message=f"Contact {params.contact_email} not found in CRM.",
                    )
                ...
                if update_res.status_code in (200, 201):
                    return ToolResult(
                        status="SUCCESS",
                        message=f"Successfully updated CRM for {params.contact_email}",
                    )
                else:
                    return ToolResult(
                        status="FAILED", message=f"HubSpot API Error: {update_res.text}"
                    )

            except Exception as e:
                return ToolResult(
                    status="FAILED",
                    message=f"Network error executing CRM update: {str(e)}",
                )

    ...
    def redact_string(cls, text: str) -> str:
        ...
        for name, pattern in cls.PATTERNS.items():
            if name == "API_KEY":
                ...
                redacted = re.sub(pattern, lambda m: m.group(0).replace(m.group(1), "[REDACTED_SECRET]"), redacted)
            else:
    ...
    def redact_dict(cls, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for k, v in data.items():
            if isinstance(v, str):
                ...
                redacted_data[k] = [cls.redact_dict(i) if isinstance(i, dict) else (cls.redact_string(i) if isinstance(i, str) else i) for i in v]
            else:
                redacted_data[k] = v

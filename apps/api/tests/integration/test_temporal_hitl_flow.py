...
from app.api.approvals import ApprovalDecision, decide_approval
...
async def test_temporal_hitl_rejection_flow(temporal_client: Client):
    ...
    async with Worker(
        ...
        workflow_id = f"test-wf-{uuid.uuid4()}"
        action_req = ActionRequest(tool_name="delete_database", arguments={}, requires_approval=True)
        ...
        await handle.signal(
            "hitl-approval-signal", 
            {"decision": "REJECTED", "edited_parameters": None, "reason": "Too dangerous"}
        )

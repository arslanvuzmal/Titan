...
async def test_tenant_isolation(monkeypatch):
    ...
    assert (
        org_b in keyword_query["args"]
    ), "Tenant Isolation Failure: Org ID not passed to FTS SQL"
    assert (
        org_b in vector_query["args"]
    ), "Tenant Isolation Failure: Org ID not passed to Vector SQL"

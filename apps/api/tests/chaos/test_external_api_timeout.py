...
async def test_external_api_timeout(monkeypatch):
    ...
    """
    # 0. Ensure tool is registered
    from app.tools.registry import ToolRegistry
    from app.tools.implementations.email_tool import EmailTool

    if not ToolRegistry.get_tool("send_email"):
    ...
    monkeypatch.setenv("SENDGRID_API_KEY", "mock_key")
    with respx.mock:
        ...
        # 3. Execute
        result = await ActionEngine.execute_action(
            req, context, agent_name="SalesAgent"
        )

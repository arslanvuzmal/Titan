...
export default function TaskDetailsPage({ params }: { params: { taskId: string } }) {
  ...
  // or we pass a generic string if auth is skipped in local dev.
  ...
  const handleDecision = async (decision: "APPROVED" | "REJECTED") => {
    try {
      await api.post(`/approvals/${params.taskId}/decision`, { decision });
    ...
    <div className="max-w-5xl mx-auto py-8">
      <div className="flex justify-between items-end mb-6">
        <div>
          ...
          <p className="text-gray-500 mt-1">ID: {params.taskId}</p>
        </div>
